/*
 * ============================================================================
 * ULTRA OPTIMIZED RIGHT HAND WIFI STREAMER WITH SPI + PROPER INTERRUPTS
 * Streams raw IMU + Flex + FSR data at 200Hz via UDP with 4-5ms latency
 * INT pin for data ready interrupts, RST for hardware recovery
 * ============================================================================
 */
...
const uint8_t ESP_ID = 2;  // Right hand
...
// FSR PIN (Force Sensitive Resistor for mode switching)
#define FSR_PIN 35

// IMU HARDWARE PINS (SPI MODE)
#define BNO08X_CS_PIN 5      // Chip Select
#define BNO08X_INT_PIN 4    // Required by library API - MUST be physically connected! (Changed from 4 to avoid FLEX_THUMB conflict)
...
// SENSOR DATA
float quat_w = 1.0, quat_x = 0.0, quat_y = 0.0, quat_z = 0.0;
float accel_x = 0.0, accel_y = 0.0, accel_z = 0.0;
float gyro_x = 0.0, gyro_y = 0.0, gyro_z = 0.0;
uint16_t flex_thumb = 0, flex_index = 0, flex_middle = 0, flex_ring = 0, flex_pinky = 0;
uint16_t fsr_value = 0;
...
// ============================================================================
// SPI POLLING MODE - NO INTERRUPT NEEDED
// ============================================================================
// SPI mode uses polling via getSensorEvent() - simpler and reliable
...
bool recoverBNO085() {
  ...
  if (!bno08x.begin_SPI(BNO08X_CS_PIN, BNO08X_INT_PIN)) {
    ...
    Serial.println("Check: PS0→3.3V, PS1→3.3V, CS→GPIO5, INT→GPIO16");
    return false;
  }
...
void checkSensorHealth() {
  ...
  // Auto-recovery disabled - it causes blocking and dropouts
  // if (packetCounter > lastPacketCount) {
  //   lastDataTime = currentTime;
  // } else if (currentTime - lastDataTime > 2000 && currentTime - lastResetTime > 5000) {
  //   Serial.println("[BNO085] ❌ No data for 2+ seconds - auto-recovering");
  //   recoverBNO085();
  //   lastDataTime = currentTime;
  // }
}
...
void setup() {
  ...
  Serial.println("🚀 ULTRA_OPTIMIZED_RIGHT_HAND_WIFI_SPI_WITH_INTERRUPTS");
  ...
  Serial.println("[HARDWARE] ✓ RST=GPIO13, CS=GPIO5");
  ...
  if (!bno08x.begin_SPI(BNO08X_CS_PIN, BNO08X_INT_PIN)) {
    ...
    Serial.println("  INT → GPIO16 (not used, but required by library)");
  ...
  Serial.println("\n🎵 ULTRA_OPTIMIZED_RIGHT_HAND_SPI_WITH_INTERRUPTS_READY 🎵");
  Serial.printf("📦 Packet size: %d bytes\n", sizeof(packet));
  Serial.printf("🎯 Target latency: 4-5ms\n");
  Serial.printf("🔄 ESP ID: %d (Right Hand)\n", ESP_ID);
  Serial.printf("📊 Mode: SPI Polling (no interrupts)\n");
...
void setupWiFi() {
  ...
  WiFi.setHostname("RightHand-ESP32");
...
// ============================================================================
// ULTRA OPTIMIZED MAIN LOOP - SPI POLLING
// ============================================================================
...
void streamData() {
  ...
  if (millis() - lastSerialOutput > 2000) {
    Serial.printf("🟦 RIGHT | Pkts: %lu | FSR: %d | WiFi: %s | RSSI: %d dBm\n", 
                  packetCounter,
                  fsr_value,
